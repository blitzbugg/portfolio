export const personalInfo = {
  name: "Ananthapadmanabhan",
  role: "Web Developer",
  summary: "Web developer passionate about building scalable, production-ready web applications with experience in React, Node.js, Next.js, TypeScript, PostgreSQL, and MongoDB. Built and deployed real-world systems including a live CMS.",
  socials: {
    github: "https://github.com/blitzbugg",
    linkedin: "https://www.linkedin.com/in/ananthapadmanabhan-m/",
    medium: "https://medium.com/@blitzbugg",
  }
};

export const experience = [
  {
    company: "Freelance",
    role: "Full Stack Developer",
    period: "2024 – Present",
    description: "Worked as a freelance developer delivering production-ready web solutions. Built and deployed a full-featured CMS for Sree Budha Central School (SBCS), including role-based dashboards, secure APIs, and optimized performance.",
  }
];

export const projects = [
  {
    title: "Official Website for a School (SBCS) 🏫",
    description: "Full CMS for real institution with role-based dashboard, PostgreSQL + Next.js, and production deployment with NGINX.",
    tech: ["Next.js", "PostgreSQL", "NGINX", "Node.js"],
    link: "https://sbcs.edu.in",
    github: "https://github.com/blitzbugg/school-cms",
  },
  {
    title: "PropEase 🏠",
    description: "Real estate platform featuring authentication, listings, and filters with a mobile-first UI.",
    tech: ["MERN Stack", "React", "MongoDB", "Tailwind"],
    link: "https://prop-ease-beryl.vercel.app/",
    github: "https://github.com/blitzbugg/propease",
  },
  {
    title: "Ezhuth 📝",
    description: "Real-time collaborative whiteboard with WebSockets + Canvas, featuring Kerala-inspired UI and typography.",
    tech: ["WebSockets", "Canvas", "React", "Node.js"],
    link: "https://ezhuth.vercel.app",
    github: "https://github.com/blitzbugg/ezhuth",
  }
];

export const skills = {
  frontend: [
    "HTML5",
    "CSS3",
    "JavaScript (ES6+)",
    "React",
    "Next.js",
    "Tailwind CSS"
  ],
  backend: [
    "Node.js",
    "Express.js",
    "TypeScript"
  ],
  database: [
    "MongoDB",
    "PostgreSQL",
    "MySQL"
  ],
  tools: [
    "Git",
    "GitHub",
    "Postman"
  ]
};

export const education = [
  {
    degree: "MCA",
    institution: "Mar Athanasios College For Advanced Studies (MACFAST)", // Placeholder
    period: "2025–2027",
    description: "Currently pursuing Master of Computer Applications.",
  },
  {
    degree: "BCA",
    institution: "Kristu Jyoti College of Management and Technology", // Placeholder
    period: "Graduated 2025",
    description: "Successfully completed Bachelor of Computer Applications.",
  }
];

export const achievements = [
  {
    title: "2nd Place – State Level Hackathon",
    issuer: "IEDC (Authentic Techmasters)",
    date: "",
    description: "Ranked second in a state-level hackathon by designing and delivering a high-impact technical solution within a competitive, time-constrained environment."
  },
  {
    title: "Technical Head",
    issuer: "National Service Scheme (NSS)",
    date: "",
    description: "Led and managed technical initiatives, overseeing execution of projects and guiding team members in development-related activities."
  }
];